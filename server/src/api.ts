...
export function createApiRouter(
  ...
  router.patch('/links/:slug', limiters.auth, wrap(async (req, res) => {
    ...
    if (body.slug !== undefined && body.slug !== link.slug) {
      ...
      // Move first. Persisting field edits before the rename commits them even
      // when the rename then fails with 409 — the caller sees an error while the
      // record has already changed. A failed rename must leave it untouched.
      const ok = await store.rename(link.slug, newSlug);
      if (!ok) return res.status(409).json({ error: 'slug_taken' });
      link.slug = newSlug;
      await store.put(link); // field edits land on the moved record
      return res.json(publicView(link, baseUrl));
    }
